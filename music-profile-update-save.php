...
$rules_array = array(
    ...
    'music_type' => array('type' => 'string', 'required' => TRUE, 'min' => 1, 'max' => 255, 'trim' => true),
...
if (sizeof($val->errors) > 0) {
    ...
    $music = $db->update('music', ['music_name' => $data['music_name'], 'address' => $data['address'], 'city' => $data['city'], 'contact_person' => $data['contact_person'], 'contact' => $data['contact'], 'email' => $data['email'], 'music_type' => $data['music_type'], 'rate' => $data['rate']], ['music_id' => $music_id]);
    ...
    if ($music >= 1) {
        echo "<script>alert('music details updated successfully!');window.location='music-profile.php'</script>";
    } else {
        echo "<script>alert('Sorry! something went wrong...');window.location='music-profile-update.php?music_id=$music_id;'</script>";
    }
}
?>

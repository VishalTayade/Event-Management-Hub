...
if (sizeof($val->errors) > 0) {
    ...
    if (move_uploaded_file($_FILES['logo_image']['tmp_name'], "./admin/logos/" . $logo_name)) {
        $img = $db->update("hotels", ['logo' => $logo_name], ['hotel_id' => $hotel_id]);
    }

    if ($hotel <= 1) {
        echo "<script>alert('Profile Updated Successfully!');window.location='hotel-profile.php';</script>";
    } else {
        echo "<script>alert('Sorry! something went wrong...');window.location='hotel-profile-update.php?hotel_id=$hotel_id';</script>";
    }
}
?>
